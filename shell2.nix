...
runCommand "openfaas-examples" {
    buildInputs = [
        ...
        #(pkgsCross.gnu64.pkgsBuildHost.haskell.packages.ghc910.ghcWithPackages (ghc: with ghc; [
        #    aeson
        #]))
        # So is this, so it's probably a bug.
        #(pkgsCross.gnu64.pkgsBuildTarget.haskell.packages.ghc910.ghcWithPackages (ghc: with ghc; [
        #    aeson
        #]))
    ...
    # 
    shellHook = ''
        # x86_64-unknown-linux-gnu-ghc src/Main.hs -ilib -o packages/sample/hello/openfaas-examples
        # rm src/Main src/Main.{hi,o}

        #cabal new-build

        #cp $(cabal exec which openfaas-examples) packages/sample/hello/openfaas-examples
        #x86_64-unknown-linux-gnu-strip packages/sample/hello/openfaas-examples
        #patchelf --set-interpreter /lib64/ld-linux-x86-64.so.2 packages/sample/hello/openfaas-examples
